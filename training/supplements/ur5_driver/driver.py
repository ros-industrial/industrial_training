            ...
            if (now - self.traj_t0) <= self.traj.points[-1].time_from_start.to_sec():
                ...
                if self.goal_handle:
                    ...
                    if position_in_tol and velocity_in_tol:
                        ...
                        rospy.loginfo("Checking for the goal.\nDesired: %s\nactual: %s\nvelocity: %s" % \
                                          (last_point.positions, state.position, state.velocity))
                        rospy.loginfo("Past trajectory endpoint, close to goal, and not moving to fast, returning success")
                    ...
                    #    rospy.logwarn("Failed to reach goal, Setting succeeded anyway")
                    #    self.goal_handle.set_succeeded()
                    #    self.goal_handle = None
        ...
        while not rospy.is_shutdown():
            ...
            if getConnectedRobot(wait=False):
                ...
                if action_server:
                    ...
                    action_server = UR5TrajectoryFollower(r, rospy.Duration(0.5))
                    action_server.start()
