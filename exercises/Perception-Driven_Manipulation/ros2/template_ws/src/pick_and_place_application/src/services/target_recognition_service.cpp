...
#include <pick_and_place_application/GetTargetPose.h>
#include <math.h>
...
{
public:
  TargetRecognition() : box_filter_scale_(BOX_SCALE), angle_tolerance_(ANGLE_TOLERANCE) {}
  ...
  bool init()
  {
    ...
    // read parameters
    if (ph.getParam("box_filter_scale", box_filter_scale_) && ph.getParam("angle_tolerance", angle_tolerance_))
    {
    ...
    target_detection_server =
        nh.advertiseService(TARGET_RECOGNITION_SERVICE, &TargetRecognition::target_recognition_callback, this);
...
protected:
  bool grab_sensor_snapshot(sensor_msgs::PointCloud2& msg)
  {
    ...
    sensor_msgs::PointCloud2ConstPtr msg_ptr =
        ros::topic::waitForMessage<sensor_msgs::PointCloud2>(SENSOR_CLOUD_TOPIC, nh, ros::Duration(5.0f));
  ...
  bool target_recognition_callback(pick_and_place_application::GetTargetPose::Request& req,
                                   pick_and_place_application::GetTargetPose::Response& res)
  {
    // transforms
  ...
  bool get_transform(std::string target, std::string source, tf::Transform& trg_to_src_tf)
  {
    ...
    try
    {
      ...
      transform_listener_ptr->lookupTransform(
          target, source, ros::Time::now() - ros::Duration(0.2f), trg_to_src_stamped);
  ...
  bool detect_box_height(const Cloud& cloud, const tf::Transform& world_to_ar_tf, double& height)
  {
    // cloud objects
