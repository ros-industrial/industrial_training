...
class GeneratePointCloud
{
	...
	public:
		GeneratePointCloud(rclcpp::Node::SharedPtr node):
		  node_(node)
		{
		}
        ...
        for(std::size_t i = 0; i < cloud_desc_node.size(); i++)
        {
          ...
          for(decltype(numeric_fields)::iterator  i = numeric_fields.begin() ; i != numeric_fields.end(); i++)
          {
            ...
            {
              std::string err_msg = boost::str(boost::format("Point Cloud description entry is missing field \"%s\"") % i->first);
		...
		void run()
		{
			rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr cloud_publisher = node_->create_publisher<sensor_msgs::msg::PointCloud2>(POINT_CLOUD_TOPIC, 1);
